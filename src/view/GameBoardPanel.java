    ...
    protected void paintComponent(Graphics g) {
        ...
        for (int row = 0; row < rows; row++) {
            for (int col = 0; col < columns; col++) {
                // Choix de couleur selon CellState
                if (cellStates[row][col] == null) cellStates[row][col] = CellState.EMPTY;

                switch (cellStates[row][col]) {
                    case EMPTY -> g2d.setColor(Color.WHITE);
                    case WALL  -> g2d.setColor(Color.GRAY);
                    case PLAYER -> g2d.setColor(Color.RED); // tu peux changer selon le joueur
                }
